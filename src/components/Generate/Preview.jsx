...
const Preview = ({ generatorId }) => {
  const { results, scheduleColor, initStore } = useGenerateStore(
    (state) => state
  );
  const { getEvent, setGetEvent, isClikedRefresh, setIsClikedRefresh } =
    useHtmlStore();
  ...
  const createDate = () => {
    ...
    for (i = 1; i <= 31; i++) {
      if (relatedResults[i]) {
        arr.push({ num: i, box: true });
      } else {
        arr.push({ num: i, box: false });
      }
      if (dateInfo.endDate.day === i) {
        break;
      }
    }
        ...
        {dates.map((date, index) => {
          return (
            <Day key={index}>
              ...
              {date.box ? (
                <Box style={{ border: "4px solid " + scheduleColor }}></Box>
              ) : (
                <></>
              )}
            </Day>
